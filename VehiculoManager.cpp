...
VehiculoManager::VehiculoManager()
    : _vehiculosArchivo(), _clientesArchivo() {
}
...
void VehiculoManager::eliminar(int idVehiculo) {
    if (_vehiculosArchivo.eliminarVehiculo(idVehiculo)) {
        ...
        for (int i = 0; i < cantidadPolizas; ++i) {
            Poliza poliza = polizaArchivo.leer(i);
            if (!poliza.getEliminado() && poliza.getIdVehiculo() == idVehiculo) {
                polizaArchivo.eliminar(i);
                cout << "POLIZA ASOCIADA ELIMINADA." << endl;
            }
        }
    } else {
...
bool VehiculoManager::recuperar(int idVehiculo) {
    if (_vehiculosArchivo.recuperarVehiculo(idVehiculo)) {
        cout << "EL VEHICULO FUE RECUPERADO CORRECTAMENTE."<<endl;
        return true;
    } else {
        cout << "NO SE PUDO RECUPERAR EL VEHICULO."<<endl;
        return false;
    }
}
...
void VehiculoManager::mostrarLista(Vehiculo vehiculo) {
    ...
    if (!cliente.getEliminado()) {
        cout << endl;
        cout << "=============================================" <<endl;
        cout <<centrar("ID DEL VEHICULO: " + to_string(vehiculo.getIdVehiculo()),45)<<endl;
        cout << "============================================="<<endl;
        cout << "CLIENTE: " << cliente.getNombre() + " " + cliente.getApellido()<<endl;
