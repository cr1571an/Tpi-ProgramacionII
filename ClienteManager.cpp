...
void ClienteManager::historialPagosPorCliente(){
  ...
  if (cantClientes==0 || cantPagos==0) return;
    ...
    if (opcion<=0 || opcion>contador){cout <<"OPERACION CANCELADA."<<endl;
        ...
        Vencimiento vencimiento = _vencimientosArchivo.leer(_vencimientosArchivo.buscarID(idsVencimiento[sel]));
        ...
        //cout << "VENCIMIENTO: " <<vencimiento.getVencimiento().formatoFecha()<<endl;
